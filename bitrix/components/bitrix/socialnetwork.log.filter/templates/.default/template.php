	...
	?><div id="sonet-log-filter" class="sonet-log-filter-block">
		...
		if (IsModuleInstalled("intranet"))
		{
			?><div class="filter-field-title-block"><?
				?><label id="filter-dest-group-tab" style="display: inline-block;" class="log-filter-field-title filter-field-title-tab<?=(empty($arResult["ToUser"]) ? "" : " webform-field-action-link")?>" onclick="onFilterDestChangeTab('group');"><?=GetMessage("SONET_C30_T_FILTER_GROUP");?></label><?
				?><span>&nbsp;/&nbsp;</span><?
				?><label id="filter-dest-user-tab" style="display: inline-block;" class="log-filter-field-title filter-field-title-tab<?=(empty($arResult["ToUser"]) ? " webform-field-action-link" : "")?>" onclick="onFilterDestChangeTab('user');"><?=GetMessage("SONET_C30_T_FILTER_USER");?></label>
			</div>			
		...
		else
		{
			...
			<span class="log-webform-field log-webform-field-textbox<?=(!$arResult["Group"]["ID"]?" log-webform-field-textbox-empty":"")?> log-webform-field-textbox-clearable">
				<span id="sonet-log-filter-group" class="log-webform-field-textbox-inner" style="width: 200px; padding: 0 20px 0 4px;">
					<input type="text" class="log-webform-field-textbox" id="filter-field-group" value="<?=$arResult["Group"]["NAME"]?>" style="height: 20px; width: 200px;" autocomplete="off" />
					<a class="sonet-log-field-textbox-clear" href=""></a>
				</span>
			</span>
			<input type="hidden" name="flt_group_id" value="<?=$arResult["Group"]["ID"]?>" id="filter_field_group_hidden">
		...
		function showLentaMenu(bindElement)
		{
			BX.PopupMenu.show(
				...
				[
					...
					if ($arParams["SHOW_EXPERT_MODE"] != "N")
					{
						?>
						,{
							text : "<?=GetMessageJS("SONET_C30_SMART_EXPERT_MODE")?>",
							className : "feed-sort-item<?=($arResult["EXPERT_MODE"] == "Y" ? " feed-sort-item-checked" : "")?>",
