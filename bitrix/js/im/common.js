;(function (window)
{
	...
	BX.MessengerCommon.prototype.decodeBbCode = function(textElement, textOnly)
	{
		...
		textElement = textElement.replace(/\[USER=([0-9]{1,})\](.*?)\[\/USER\]/ig, function(whole, userId, text)
		{
			...
			if (!textOnly && text && userId > 0 && typeof(BXIM) != 'undefined')
	...
	BX.MessengerCommon.prototype.chatListPrepare = function(params)
	{
		...
		for (var i = 0; i < category.length; i++)
		{
			...
			groupElements[i] = [];
			if (category[i].id == 'private' || category[i].id == 'extranet' || category[i].id == 'blocked' || category[i].id == 'bot')
			{
		...
		for (var i = 0; i < category.length; i++)
		{
			...
			if (groupElements[i].length <= 0 && (category[i].id == 'call' || category[i].id == 'extranet' || category[i].id == 'blocked' || category[i].id == 'bot'))
				continue;

			chatList.appendChild(BX.create("div", {props : { className: "bx-messenger-chatlist-group"}, children : [
				(category[i].id == 'call' || category[i].id == 'blocked' || category[i].id == 'bot')? null: BX.create("span", {attrs: {'data-type': category[i].id}, props : { title: category[i].title, className: "bx-messenger-chatlist-group-add"}}),
			...
			for (var j = 0; j < groupElements[i].length; j++)
			{
				...
				if (category[i].id == 'private' || category[i].id == 'extranet' || category[i].id == 'bot')
				{
