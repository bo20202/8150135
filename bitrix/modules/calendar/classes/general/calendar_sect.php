...
class CCalendarSect
{
	...
	public static function GetList($Params = array())
	{
		...
		{
			$cache = new CPHPCache;
			$cacheId = serialize(array('section_list', $arFilter, $arOrder, $Params['joinTypeInfo'], CCalendar::IsIntranetEnabled())).($checkPermissions ? $userId.'_'.CCalendar::IsSocnetAdmin() : '').($getPermissions ? '_get_perm' : '');
			...
			if ($cache->InitCache(CCalendar::CacheTime(), $cacheId, $cachePath))
			{
				...
				{
					self::$Permissions = array_merge(self::$Permissions, $res["permissions"]);
				}
			}
		}
		...
		if (($checkPermissions || $getPermissions) && count($arSectionIds) > 0)
		{
			$userId = $Params['userId'] ? intVal($Params['userId']) : CCalendar::GetCurUserId();
			$res = array();
			$arAccessCodes = array();
