...
class CIMChat
{
	...
	public function SetOwner($chatId, $userId, $checkPermission = true)
	{
		...
		IM\ChatTable::update($chatId, Array('AUTHOR_ID' => $userId));
	...
	public function Add($arParams)
	{
		...
		{
			$chatDescription = CIMMessenger::PrepareMessageForPush(Array(
		...
		if ($chatId > 0)
		{
			...
			if (count($arUsersName) > 1 && !$skipUserAdd)
			{
	...
	public function AddUser($chatId, $userId, $hideHistory = true)
	{
		global $DB;
		...
		$strSql = "
			SELECT
				C.ID CHAT_ID, C.TITLE CHAT_TITLE, C.AUTHOR_ID CHAT_AUTHOR_ID, C.EXTRANET CHAT_EXTRANET, C.TYPE CHAT_TYPE
			FROM b_im_chat C
			WHERE
				C.TYPE = '".IM_MESSAGE_OPEN."' AND C.ID = ".$chatId."
		";
		$dbRes = $DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__);
		$arRes = $dbRes->Fetch();
		if (!$arRes)
