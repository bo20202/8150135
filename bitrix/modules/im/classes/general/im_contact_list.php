...
class CAllIMContactList
{
	...
	static function GetUserData($arParams = Array())
	{
		...
		if($useCache)
		{
			...
			$cache_id = 'user_data_v10_'.(is_object($USER)? $USER->GetID(): 'AGENT').'_'.implode('|', $arFilter['=ID']).'_'.$nameTemplate.'_'.$nameTemplateSite.'_'.$getPhones.'_'.$getDepartment.'_'.$bIntranetEnable.'_'.$bVoximplantEnable.'_'.LANGUAGE_ID.'_'.$bColorEnabled;
	...
	public static function GetRecentList($arParams = Array())
	{
		...
		$cache_ttl = 2592000;
		$cache_id = 'im_recent_v9_'.$userId.'_'.$bColorEnabled.'_'.$bOpenChatEnabled;
