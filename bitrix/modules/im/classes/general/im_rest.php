...
class CIMRestService extends IRestService
{
		...
		return array(
			...
			'imbot' => Array(
				...
				CRestUtil::EVENTS => array(
					'OnImBotMessageAdd' => array('im', 'onImBotMessageAdd', array(__CLASS__, 'onBotMessageAdd'), array("category" => \Bitrix\Rest\Sqs::CATEGORY_BOT)),
					...
					'OnImCommandAdd' => array('im', 'onImCommandAdd', array(__CLASS__, 'onCommandAdd'), array("category" => \Bitrix\Rest\Sqs::CATEGORY_BOT)),
				),
			)
		);
	}
	...
	public static function OnRestAppDelete($arParams)
	{
		...
		$result = \CBitrix24App::getList(array(), array('ID' => $arParams['APP_ID']));
		if ($result = $result->fetch())
		{
			$bots = \Bitrix\Im\Bot::getListCache();
			foreach ($bots as $bot)
			{
				if ($bot['APP_ID'] == $result['APP_ID'])
				{
	...
	public static function botRegister($arParams, $n, CRestServer $server)
	{
		...
		{
			throw new Bitrix\Rest\RestException("Module \"Bitrix24\" isn't installed", "BITRIX24_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		}

		$dbRes = CBitrix24App::GetList(array(), array('APP_ID' => $server->getAppId()));
		$arApp = $dbRes->Fetch();
		...
		if ($botId)
		{
			self::unbindEvent($arApp['ID'], $arApp['APP_ID'], 'im', 'onImBotMessageAdd', 'OnImBotMessageAdd', true);
			self::bindEvent($arApp['ID'], 'im', 'onImBotMessageAdd', 'OnImBotMessageAdd', $arParams['EVENT_MESSAGE_ADD']);

			self::unbindEvent($arApp['ID'], $arApp['APP_ID'], 'im', 'onImBotJoinChat', 'OnImBotJoinChat', true);
			self::bindEvent($arApp['ID'], 'im', 'onImBotJoinChat', 'OnImBotJoinChat', $arParams['EVENT_WELCOME_MESSAGE']);

			self::unbindEvent($arApp['ID'], $arApp['APP_ID'], 'im', 'onImBotDelete', 'OnImBotDelete', true);
	...
	public static function botUpdate($arParams, $n, CRestServer $server)
	{
		...
		if (isset($arParams['FIELDS']['TYPE']) && !empty($arParams['FIELDS']['TYPE']))
		{
	...
	public static function commandRegister($arParams, $n, CRestServer $server)
	{
		...
		if ($commandId)
		{
			self::unbindEvent($arApp['ID'], $arApp['APP_ID'], 'im', 'onImCommandAdd', 'onImCommandAdd', true);
	...
	private static function unbindEvent($appId, $appCode, $bitrixEventModule, $bitrixEventName, $restEventName, $skipCheck = false)
	{
		if (!$skipCheck)
		{
			$res = \Bitrix\Im\BotTable::getList(array(
				'filter' => array(
					'=APP_ID' => $appCode,
				),
				'select' => array('BOT_ID')
