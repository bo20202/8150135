...
class Bot
{
	...
	public static function register(array $fields)
	{
		...
		$result = \Bitrix\Im\BotTable::add(Array(
			'BOT_ID' => $botId,
	...
	public static function unRegister(array $bot)
	{
		...
		\Bitrix\Im\BotTable::delete($botId);

		$orm = \Bitrix\Im\BotChatTable::getList(Array(
			'filter' => Array('=BOT_ID' => $botId)
		));
		if ($row = $orm->fetch())
		{
			\Bitrix\Im\BotChatTable::delete($row['ID']);
		}
		...
		$orm = \Bitrix\Im\CommandTable::getList(Array(
			'filter' => Array('=BOT_ID' => $botId)
		));
		while ($row = $orm->fetch())
		{
	...
	public static function update(array $bot, array $updateFields)
	{
		...
		if (!empty($update))
		{
			\Bitrix\Im\BotTable::update($botId, $update);
	...
	public static function onJoinChat($dialogId, $joinFields)
	{
		...
		if ($bot["METHOD_WELCOME_MESSAGE"] && class_exists($bot["CLASS"]) && method_exists($bot["CLASS"], $bot["METHOD_WELCOME_MESSAGE"]))
		{
			...
			\Bitrix\Im\BotTable::update($joinFields['BOT_ID'], $updateCounter);
	...
	public static function getListCache()
	{
		...
		{
			$result = Array();
			$orm = \Bitrix\Im\BotTable::getList();
			while ($row = $orm->fetch())
			{
	...
	public static function changeChatMembers($chatId, $botId, $append = true)
	{
		...
		if ($append)
		{
			...
			\Bitrix\Im\BotChatTable::add(Array(
				'BOT_ID' => $botId,
				'CHAT_ID' => $chatId
			));
		}
