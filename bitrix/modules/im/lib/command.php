...
class Command
{
	...
	public static function register(array $fields)
	{
		...
		$result = \Bitrix\Im\CommandTable::add(Array(
			'BOT_ID' => $botId,
			'MODULE_ID' => $moduleId,
			'COMMAND' => $command,
			'COMMON' => $common,
		...
		if ($moduleId == 'rest')
		{
			foreach ($langSet as $lang)
			{
				...
				try
				{
					\Bitrix\Im\CommandLangTable::add(array(
	...
	public static function unRegister(array $command)
	{
		...
		\Bitrix\Im\CommandTable::delete($commandId);

		$orm = \Bitrix\Im\CommandLangTable::getList(Array(
			'filter' => Array('=COMMAND_ID' => $commandId)
		));
		while ($row = $orm->fetch())
		{
			\Bitrix\Im\CommandLangTable::delete($row['ID']);
		}
	...
	public static function update(array $command, array $updateFields)
	{
		...
		if (!empty($update))
		{
			\Bitrix\Im\CommandTable::update($commandId, $update);
	...
	public static function onCommandAdd($messageId, $messageFields)
	{
		...
		foreach ($commandList as $params)
		{
			...
			if ($params["METHOD_COMMAND_ADD"] && class_exists($params["CLASS"]) && method_exists($params["CLASS"], $params["METHOD_COMMAND_ADD"]))
			{
				if ($params['BOT_ID'] > 0)
				{
					\Bitrix\Im\BotTable::update($params['BOT_ID'], array(
	...
	public static function addMessage(array $access, array $messageFields)
	{
		...
		$orm = \Bitrix\Im\MessageTable::getById($messageId);
	...
	public static function getListCache($lang = LANGUAGE_ID)
	{
		...
		else
		{
			$loadRestLang = false;
			$result = Array();
			$orm = \Bitrix\Im\CommandTable::getList();
			while ($row = $orm->fetch())
			{
			...
			{
				$langSet = Array();
				$orm = \Bitrix\Im\CommandLangTable::getList();
				while ($row = $orm->fetch())
				{
	...
	public static function getAccessToken($botId, $dialogId, $prolong = false)
	{
		...
		if (!$result[$dialogId] || $result[$dialogId]['DATE_EXPIRE'] < $date->getTimestamp())
		{
			...
			$orm = \Bitrix\Im\BotTokenTable::add(Array(
		...
		{
			$date = new \Bitrix\Main\Type\DateTime();
			$orm = \Bitrix\Im\BotTokenTable::update($result[$dialogId]['ID'], Array(
				'DATE_EXPIRE' => $date->add('10 MINUTES')
			));
			if ($orm->isSuccess())
			{
	...
	private static function getAccessTokenCache($botId)
	{
		...
		{
			$result = Array();
			$orm = \Bitrix\Im\BotTokenTable::getList(Array(
				'filter' => array(
