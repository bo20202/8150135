	...
	protected function processActionSetResponsible()
	{
		...
		else
		{
			...
			elseif(count($templateData) > 1)
			{
				...
				{
					$url = 'javascript:BX[\'LiveFeedClass_'.$this->randomString.'\'].setResponsible(\''.$templateId.'\');';
				...
				if($field["MULTIPLE"] == "Y"  && $field["TYPE"] != "S:DiskFile")
				{
					...
					foreach($this->lists['FORM_DATA'][$fieldId] as $key => $value)
					{
						if($field["TYPE"] == "S:HTML")
						{
							...
							$html .= '<tr><td><input type="hidden" name="'.$fieldId.'['.$key.'][VALUE][TYPE]" value="html">'
		...
		?><table class="bx-lists-table-content <?= $classTable ?>">
			...
			<? foreach($dataArray as $field):
				...
				if($method == 'bizproc' && $manyParameters): ?>
					<? if(!$listTemplateId[$field['templateId']]): ?>
				...
				?>
				<tr <?= $style ?>>
					<td><?=htmlspecialcharsbx($field['name'])?>: <?= $required ?></td>
					<?
					switch($field['type']):
						case 'label':
						case 'custom':
