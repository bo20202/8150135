		...
		if (preg_match_all('|\[DISK\sFILE\sID=([n]*\d+)\]|', $text, $matches))
		{
			...
			foreach($matches[1] as $inlineFileId)
			{
				if (!empty($attachmentUrlList[$inlineFileId]))
				{
					...
					$arSourceSize = $arDestinationSize = array();
					\CFile::scaleImage(
	...
	public static function processBlogPostShare($fields, $params)
	{
		...
		if (Loader::includeModule('blog'))
		{
			...
			if(\CBlogPost::update($postId, array("SOCNET_RIGHTS" => $sonetRights, "HAS_SOCNET_ALL" => "N")))
			{
				...
				$arSPERM = \CBlogPost::getSocnetPermsName($postId);
				$extranet = Loader::includeModule("extranet");
				$bExtranetSite = ($extranet && \CExtranet::isExtranetSite());
				...
				$arIntranetUserID = ($extranet ? \CExtranet::getIntranetUsers() : false);
					...
					foreach($v as $vv)
					{
						...
						)
						{
							if($arSocNetGroup = \CSocNetGroup::getByID($vv["ENTITY_ID"]))
							{
								...
								if ($groupSiteID)
								{
									$arTmp = \CSocNetLogTools::processPath(array("GROUP_URL" => $link), $userId, $groupSiteID); // user_id is not important parameter
						...
						elseif ($type == "U")
						{
							...
							elseif(in_array($type.$vv["ENTITY_ID"], $newRights))
							{
								...
								$name = \CUser::formatName($params["NAME_TEMPLATE"], $arTmpUser, ($params["SHOW_LOGIN"] != "N" ? true : false));
								$id = $vv["ENTITY_ID"];
								$link = \CComponentEngine::makePathFromTemplate(
									$params["PATH_TO_USER"],
									array(
				...
				if($commentId = \CBlogComment::add($arComFields))
				{
